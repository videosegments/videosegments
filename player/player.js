...
class Player {
  constructor(video, mutePlayEvent) {
    ...
    this.id = window.location.href.match(
    ...
    if (settings.hideEndScreenCards === 'yes') {
      ...
      injectCSqSRule('.ytp-ce-playlist {visibility: hidden !important;}');
  ...
  async getSegmentation() {
    ...
    let progressBar =
      ...
      document.getElementsByClassName('no-model cue-range-markers')[0];
    ...
    if (pid === null) {
      ...
      if (q_segments != null) {
        ...
        this.getLocalSegmentation().then((segmentation) =>
          self.onGotSegmentation('local', segmentation, 'official')
        );
      }
    } else {
  ...
  onGotSegmentation(origin, segmentation, secondaryOrigin) {
    ...
    log(
      'got ' +
        (settings.databasePriority === origin ? 'primary' : 'secondary') +
        ' segmentation:',
      origin,
      segmentation
    );
    ...
    // save this segmentation as secondary
      ...
      if (
        typeof this[settings.databasePriority] !== 'undefined' &&
        typeof this[settings.databasePriority].types === 'undefined' &&
        typeof segmentation.types !== 'undefined'
      ) {
        // set secondary segmentation as primary
        this.segmentation = segmentation;
        this.segmentation.origin = origin;
  ...
  async onSegmentationReady() {
    ...
    if (this.segmentation.origin === 'noSegmentation') {
      this.segmentation = await tryChannelFilter(
        this.channel,
        this.video.duration
      );
    } else {
      ...
      if (
        typeof this.segmentation.types !== 'undefined' &&
        this.segmentation.types[this.segmentation.types.length - 1] === '-'
      ) {
    ...
    this.segmentsbar.set(
      this.segmentation.timestamps,
      this.segmentation.types,
      this.video.duration
    );
  ...
  getCommunitySegmentation() {
    ...
    return new Promise((resolve) => {
      ...
      xhr.open(
        'GET',
        'https://db.videosegments.org/api/v3/get.php?id=' + this.id
      );
  ...
  async getPendingSegmentation(pid) {
    let response = await xhr_get(
      'https://db.videosegments.org/api/v3/review.php?id=' + pid
    );
    if (
      typeof response.timestamps !== 'undefined' &&
  ...
  getSimplifiedSegmentation(segmentation) {
    ...
    if (
      this.getSegmentSimplifiedType(
        segmentation.types[segmentation.types.length - 1]
      ) === lastType
    ) {
      simplified.timestamps.push(
        segmentation.timestamps[segmentation.timestamps.length - 1]
      );
      simplified.types.push(lastType);
    }
  ...
  getOriginalSegmentation(segmentation) {
    if (
      typeof this.originalSegmentation === 'undefined' ||
      typeof this.originalSegmentation.timestamps === 'undefined'
    ) {
      return {
        timestamps: segmentation.timestamps,
  ...
  tryRewind(toSegmentNumber) {
    ...
    if (delay <= 0) {
      let duration =
        this.segmentation.timestamps[toSegmentNumber + 1] - currentTime;
      if (
        duration <=
        settings.segments[this.segmentation.types[toSegmentNumber]].duration
      ) {
        this.prevPlaybackRate = this.video.playbackRate;
        this.preventRateChangedEvent = true;

        this.video.playbackRate =
          settings.segments[this.segmentation.types[toSegmentNumber]].speed;
        ...
        this.video.currentTime =
          this.segmentation.timestamps[toSegmentNumber + 1];

        toSegmentNumber = this.findNextSegmentToRewind(toSegmentNumber);
        delay =
          this.segmentation.timestamps[toSegmentNumber] -
          this.video.currentTime;
      }
    }
  ...
  findNextSegmentToRewind(currentSegmentNumber) {
    ...
    for (
      let i = currentSegmentNumber;
      i < this.segmentation.types.length;
      ++i
    ) {
      if (
        settings.segments[this.segmentation.types[i]].skip == true &&
        this.segmentation.timestamps[i] >= currentTime
      ) {
        return i;
      }
    }
    ...
    if (prop === 'databasePriority') {
      ...
      if (settings.mode === 'simplified') {
        if (value.segment === 'pl' || value.segment === 'sk') {
          this.segmentsbar.updateColor(
            value.segment,
            settings.segments[value.segment].color,
            settings.segments[value.segment].opacity
          );
        }
      } else {
        if (value.segment !== 'pl' && value.segment !== 'sk') {
